package LabExercises.lab11.ex2;
...
public class Faculty {
    public static void main(String[] args) {
        ...
        AdjacencyListGraph<String> adjacencyMatrixGraph = new AdjacencyListGraph<>();
        for (int i = 0; i < N; i++) {
            String s = sc.nextLine();
            adjacencyMatrixGraph.addVertex(s);
        }
        int M = Integer.parseInt(sc.nextLine());
        for (int i = 0; i < M; i++) {
            String[] input = sc.nextLine().split(" ");
            for (int j = 1; j < input.length; j++) {
                adjacencyMatrixGraph.addEdge(input[0], input[j]);
            }
        }

        String subject = sc.nextLine();
        List<String> topOrder = adjacencyMatrixGraph.topologicalSort();
        Collections.reverse(topOrder);
        System.out.println(topOrder.get(topOrder.indexOf(subject) + 1));
    }
}
