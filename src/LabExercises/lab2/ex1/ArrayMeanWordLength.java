...
public class ArrayMeanWordLength {
    ...
    public static String wordClosestToAverageLength(Array<String> arr) {
        ...
        double avg = sum * 1.0 / arr.getSize();
        String max = arr.get(0);
        double diff = Math.abs(max.length() - avg);
        for (int i = 0; i < arr.getSize(); i++) {
            String temp = arr.get(i);
            double diff1 = Math.abs(temp.length() - avg);
            if (diff1 < diff || (diff1 == diff) && temp.length() > max.length()) {
                max = temp;
                diff = diff1;
            }
        }
